    ...
    context('when file2 content is invalid', () => {
        ...
        async function assertBlankFileContent(fileContent2?: string) {
            const response = await request(server)
                .post(ApiRoutes.alteration())
                .send({
                    scenario: 'replay all_planes from_recording "record_AFR.sbs" from 0 seconds until 1000 seconds ',
                    fileContent: 'MSG,4,3,5022202,4CA1FA,5022202,2018/11/25,11:30:48.179,2018/11/25,11:30:48.179,,,474.53,295.86,,,0.0,,,,,',
                    fileName: 'myfile.sbs',
                    fileContent2,
    ...
    context('when file2 name is invalid', () => {
        ...
        async function assertBlankFileName(fileName2?: string) {
            const response = await request(server)
                .post(ApiRoutes.alteration())
                .send({
                    scenario: 'replay all_planes from_recording "myfile2.sbs" from 0 seconds until 1000 seconds ',
                    fileContent: 'MSG,4,3,5022202,4CA1FA,5022202,2018/11/25,11:30:48.179,2018/11/25,11:30:48.179,,,474.53,295.86,,,0.0,,,,,',
                    fileName: 'myfile.sbs',
                    fileContent2: 'MSG,4,3,5022202,4CA1FA,5022202,2018/11/25,11:30:48.179,2018/11/25,11:30:48.179,,,474.53,295.86,,,0.0,,,,,',
                    fileName2
                })
                .expect(422)
    ...
    context('when all is valid', () => {
        it('returns 200 and altered recordings', async () => {
            const response = await request(server)
                .post(ApiRoutes.alteration())
                .send({
                    scenario: 'let $call = {0, 10}, hide all_planes from $call seconds until 1000 seconds ',
                    recording: {
