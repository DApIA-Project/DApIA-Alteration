...
export class TypescriptAlterationManager implements IAlterationManager {
  ...
  runAlterations(
    ...
    if (recordingToReplay)
      this.replayRecording = parse(recordingToReplay!.content)
      ...
      resolve(
        parameters.map((param, i) => {
          ...
          let engine = new Engine({
            actions:
              sensor?.action?.map((a) =>
                this.create_action(a, start_date, source_recording)
              ) ?? [],
          })

          let name =
            'modified__' + recording.name.split('.')[0] + '_' + i + '.sbs'
  ...
  create_action(
    ...
    switch (action.alterationType) {
      ...
      case 'DELETION': {
        let frequency_parameter = action.parameters.parameter?.find(
          (arg) => arg.frequency != undefined
        )
        let frequency = 0
      ...
      case 'ROTATION': {
        let angle_parameter = action.parameters.parameter?.find(
          (arg) => arg.angle != undefined
        )
      ...
      case 'REPLAY': {
        if (!action.parameters.recordPath)
          throw new Error('Invalid parameters for DELETION engine')
        ...
        let alterations = action.parameters.parameter
          ?.map((arg) => {
            ...
            return {
              ...
              value: value,
              mode: <AlterationMode>(
                (arg.mode == 'simple' ? 'replace' : arg.mode)
              ),
            }
          })
          .filter((alt) => alt != null)
      ...
      case 'SATURATION': {
        let aircraft_number = action.parameters.parameter?.find(
          (p) => p.key == 'AIRCRAFT_NUMBER'
        )?.number

        if (!aircraft_number) {
          throw new Error(
            'Saturation Engine Error : "AIRCRAFT_NUMBER" parameter is not defined'
          )
        }

        let icao = action.parameters.parameter
          ?.find((p) => p.key == 'ICAO')
          ?.value?.toLowerCase()
      ...
      case 'TRAJECTORY': {
        ...
        if (!action.parameters.trajectory)
          throw new Error(
            'Trajectory modification requires waypoints parameters'
          )
      ...
      case 'ALTERATIONTIMESTAMP': {
        let time = action.parameters.parameter?.find(
          (p) => p.key == 'timestamp'
        )?.value!
        return delay({
          scope: scope,
